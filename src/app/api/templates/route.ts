...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, content, campaign, isDefault } = await request.json();
    if (!name || !content) {
    ...
    const template = await db.smsTemplate.create({ data: { name, content, campaign: campaign || null, isDefault: !!isDefault } });
