...
export default function DashboardView() {
  ...
  return (
    <div className="space-y-6">
      {/* Stats Grid */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Total Clients" value={summary?.totalClients || 0} icon={Users} color="bg-blue-600" trend={12} />
        <StatCard title="SMS This Month" value={summary?.smsSentThisMonth || 0} icon={MessageSquare} color="bg-emerald-600" trend={8} />
        <StatCard title="Today's Birthdays" value={summary?.todaysBirthdays || 0} icon={Cake} color="bg-purple-600" />
        <StatCard title="Success Rate" value={`${summary?.failedSms != null && summary?.smsSentThisMonth > 0 ? Math.round(((summary.smsSentThisMonth - summary.failedSms) / summary.smsSentThisMonth) * 100) : 100}%`} icon={TrendingUp} color="bg-teal-600" trend={3} />
      </div>
