...
import { apiFetch } from '@/store/app';
...
export default function ReportsView() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      if (dRes.success) {
        // API returns flat object { total, sent, delivered, failed, pending, successRate }
      ...
      if (hRes.success) {
        ...
        const logs = Array.isArray(hData?.logs) ? hData.logs : (Array.isArray(hData) ? hData : []);
        setHistory(logs);
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold text-white">{(() => { const s = summary; if (!s) return 0; const total = (s.smsSentThisMonth || 0); const failed = (s.failedSms || 0); return total > 0 ? Math.round(((total - failed) / total) * 100) : 0; })()}%</p>
        ...
        <Card className="bg-slate-900 border-slate-800">
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold text-white">{summary?.birthdaysToday || 0}</p>
